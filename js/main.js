...
/* BEGIN PATCH: loadReputationHome (star + keep original order) */
async function loadReputationHome(){
    ...
    // 只取 star=true；保持 JSON 原始顺序；不按 year 排序
    const raw = Array.isArray(rep.items) ? rep.items : [];
    const starred = raw.filter(it => it && it.star === true);

    const items = starred.slice(0, K);
